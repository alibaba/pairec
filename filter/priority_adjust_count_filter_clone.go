...
func (f *PriorityAdjustCountFilter) CloneWithConfig(params map[string]interface{}) IFilter {
	...
	config.Name = f.name
	cloneFilter := NewPriorityAdjustCountFilter(config)
	if cloneFilter != nil {
...
func (f *PriorityAdjustCountFilter) GetFilterName() string {
	return f.name
}
