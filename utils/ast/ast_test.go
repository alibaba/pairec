...
func TestGoAntlrEvaluateStatement(t *testing.T) {
	cdn_probs_ctr := 0.004212516359984875
	cdn_probs_cvr := 0.0014530600747093558
	log_price := 0.6931471805599453
	ret := (cdn_probs_ctr + 2*cdn_probs_cvr) * (math.Pow(log_price, 0.1))
	expression := "fx=(${cdn_probs_ctr}+2*${cdn_probs_cvr})*${log_price}^0.1;"
	ast, err := valuate.NewEvaluableStatement(expression)
	if err != nil {
		t.Fatal(err)
	}
	...
	data := item.ExprData()
	result, errs := ast.Evaluate(data)
	if len(errs) != 0 {
		t.Fatal(errs)
	}
	if utils.ToFloat(result["fx"], float64(0)) != ret {
		t.Log(result)
		t.Fatal("result not equal")
	}
}

func TestGoAntlrEvaluateStatement2(t *testing.T) {
	cdn_probs_ctr := 0.004212516359984875
	cdn_probs_cvr := 0.0014530600747093558
	log_price := 0.6931471805599453
	ret := (cdn_probs_ctr + 2*cdn_probs_cvr) * (math.Pow(log_price, 0.1))
	expression := "fx=(${cdn_probs_ctr}+2*${cdn_probs_cvr})*${log_price}^0.1;"
	ast, err := GetExpASTByAntlrWithStatement(expression, "fx")
	if err != nil {
		t.Fatal(err)
	}
