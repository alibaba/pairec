...
var cachesByAntlrWithStatement = make(map[string]ExprAST)
...
type exprASTWithStatement struct {
	expression  *valuate.EvaluableStatement
	returnParam string
}

func (e *exprASTWithStatement) Evaluate(data map[string]any) (result float64, err error) {
	defer func() {
		if r := recover(); r != nil {
			stack := string(debug.Stack())
			log.Error(fmt.Sprintf("error=%v, stack=%s", err, strings.ReplaceAll(stack, "\n", "\t")))
	...
	dataMap, errs := e.expression.Evaluate(data)
	if len(errs) != 0 {
		err = errs[0]
		return
	}
	if ret, ok := dataMap[e.returnParam]; ok {
		if r, ok := ret.(float64); ok {
			result = r
			return
		} else {
		...
		result = float64(0)
		err = fmt.Errorf("expression result not found param:%v", e.returnParam)
		return
	}
}
func (e *exprASTWithStatement) toStr() string {
	return ""
}

func GetExpASTByAntlrWithStatement(source, returnParam string) (ExprAST, error) {
	if source == "" {
		return nil, nil
	}
	//var exprAst ExprAST
	mutex.RLock()
	exprAst, ok := cachesByAntlrWithStatement[source]
	mutex.RUnlock()
	if !ok {
		expression, err := valuate.NewEvaluableStatement(source)
		if err != nil {
			return nil, err
		}
		exprAst = &exprASTWithStatement{
			expression:  expression,
			returnParam: returnParam,
		}
		mutex.Lock()
		cachesByAntlrWithStatement[source] = exprAst
		mutex.Unlock()
	}

	return exprAst, nil
}
func ExprASTResultByAntlrWithStatement(expr ExprAST, exprDatas ParameterExprData) float64 {
	switch ast := expr.(type) {
	// 传入的根节点是 BinaryExprAST
	case *exprASTWithStatement:
		data := exprDatas.ExprData()
		result, err := ast.Evaluate(data)
		if err != nil {
			log.Error(fmt.Sprintf("expression invoke error:%v", err))
