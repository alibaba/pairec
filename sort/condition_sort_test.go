...
func TestConditionSort_InvalidDataType_ReturnsError(t *testing.T) {
	config := recconf.SortConfig{
		Name:     "test_condition_sort_3",
		SortType: "ConditionSort",
	}
	condSort := NewConditionSort(config)

	ctx := context.NewRecommendContext()
	ctx.RecommendId = "test_req_3"

	sortData := &SortData{
		Data:    "invalid_data", // Not []*module.Item
		Context: ctx,
		User:    module.NewUser("test_user"),
	}
...
func TestConditionSort_GetSortName(t *testing.T) {
	config := recconf.SortConfig{
		Name:     "my_condition_sort",
		SortType: "ConditionSort",
	}
	condSort := NewConditionSort(config)

	if condSort.GetSortName() != "my_condition_sort" {
		t.Errorf("GetSortName() = %v, want %v", condSort.GetSortName(), "my_condition_sort")
	}
}

func TestConditionSort_CloneWithConfig(t *testing.T) {
	config := recconf.SortConfig{
		Name:     "clone_test_sort",
		SortType: "ConditionSort",
	}
	condSort := NewConditionSort(config)

	cloned := condSort.CloneWithConfig(map[string]interface{}{"Name": "new_name"})
	if cloned == nil {
