...
func NewFeatureClickHouseDao(config recconf.FeatureDaoConfig) *FeatureClickHouseDao {
	dao := &FeatureClickHouseDao{
		...
		table:              config.ClickHouseTableName,
	...
	clickHouseDB, err := clickhouse.GetClickHouse(config.ClickHouseName)
	if err != nil {
		log.Error(fmt.Sprintf("error=%v", err))
		return nil
	}
	dao.db = clickHouseDB.DB
	return dao
}
func (d *FeatureClickHouseDao) getItemStmt(key int) *sql.Stmt {
	d.mu.RLock()
	defer d.mu.RUnlock()
...
func (d *FeatureClickHouseDao) userFeatureFetch(user *User, context *context.RecommendContext) {
	defer func() {
		if err := recover(); err != nil {
			log.Error(fmt.Sprintf("requestId=%s\tmodule=FeatureClickHouseDao\terror=%v", context.RecommendId, err))
			return
		}
	}()
	...
	if key == "" {
		log.Error(fmt.Sprintf("requestId=%s\tmodule=FeatureClickHouseDao\terror=property not found(%s)", context.RecommendId, comms[1]))
		return
	}
	...
	builder.Where(builder.Equal(d.userFeatureKeyName, key)).Limit(1)
	...
	if d.userStmt == nil {
		...
		if d.userStmt == nil {
			...
			if err != nil {
				log.Error(fmt.Sprintf("requestId=%s\tmodule=FeatureClickHouseDao\terror=clickhouse error(%v)", context.RecommendId, err))
				d.mu.Unlock()
				return
			}
			d.userStmt = stmt
	...
	if err != nil {
		if errors.Is(err, gocontext.DeadlineExceeded) {
			log.Warning("module=FeatureClickHouseDao\tevent=userFeatureFetch\ttimeout=100")
			return
		}
		log.Error(fmt.Sprintf("requestId=%s\tmodule=FeatureClickHouseDao\terror=clickhouse error(%v)", context.RecommendId, err))
		return
...
func (d *FeatureClickHouseDao) itemsFeatureFetch(items []*Item, context *context.RecommendContext) {
	defer func() {
		if err := recover(); err != nil {
			log.Error(fmt.Sprintf("requestId=%s\tmodule=FeatureClickHouseDao\terror=%v", context.RecommendId, err))
			return
			...
			case itemlist := <-requestCh:
				...
				go func() {
					...
					case <-ctx.Done():
						...
						log.Error(fmt.Sprintf("requestId=%s\tmodule=FeatureClickHouseDao\terror=%v", context.RecommendId, ctx.Err()))
						return
					default:
					}
					rowsChannel <- rows
				...
				select {
				case <-ctx.Done():
					log.Error(fmt.Sprintf("requestId=%s\tmodule=FeatureClickHouseDao\terror=clickhouse error(%v)", context.RecommendId, ctx.Err()))
					return
