...
func NewUserCusteomRecallHologresDao(config recconf.RecallConfig) *UserCustomRecallHologresDao {
	...
	dao := &UserCustomRecallHologresDao{
...
func (d *UserCustomRecallHologresDao) ListItemsByUser(user *User, context *context.RecommendContext) (ret []*Item) {
	...
	if d.userStmt == nil {
		...
		if d.userStmt == nil {
			...
			if err != nil {
				log.Error(fmt.Sprintf("requestId=%s\tmodule=UserCustomRecallHologresDao\terror=hologres error(%v)", context.RecommendId, err))
				d.mu.Unlock()
				return
			}
			d.userStmt = stmt
	...
	if err != nil {
		log.Error(fmt.Sprintf("requestId=%s\tmodule=UserCustomRecallHologresDao\tuid=%s\terror=%v", context.RecommendId, uid, err))
		return
	}
